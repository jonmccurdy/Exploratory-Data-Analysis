...
$$ 
\text{test statistic} = \frac{\hat{p} - p_0}{SE} = \frac{\hat{p} - p_0}{\sqrt{\frac{p_0(1-p_0)}{n}}}
$$
...
$$
SE_{\hat{p_1} - \hat{p_2}} = \sqrt{SE^2_{\hat{p_1}} + SE^2_{\hat{p_2}}} = \sqrt{\frac{\hat{p_1}(1-\hat{p_1})}{n_1} + \frac{\hat{p_2}(1-\hat{p_2})}{{n_2}}}
$$
...
$$
SE_{\hat{p_1} - \hat{p_2}} = \sqrt{\overline{p}(1-\overline{p})\left( \frac{1}{n_1} + \frac{1}{n_2}\right)} \qquad \text{where} \qquad \overline{p}=\frac{\hat{p_1}n_1 + \hat{p_2}n_2}{n_1 + n_2}
$$
...
$$
H_0: p_A = p_B \qquad \text{and} \qquad H_A: p_A \neq p_B
$$

Since we are testing to see if there is no difference between the proportions we will use the pooled standard error:

$$
SE_{\hat{p_A} - \hat{p_B}} = \sqrt{ \frac{50 + 60}{500 + 450} \left( 1- \frac{50 + 60}{500 + 450} \right)\left(\frac{1}{500} + \frac{1}{450}\right)} = 0.02079
$$

We can then calculate the test statistic as:

$$
\frac{\hat{p_B} - \hat{p_A} - (p_B - p_A)}{SE_{\hat{p_A} - \hat{p_B}}} = \frac{\frac{60}{450}-\frac{50}{500}}{0.02079} = 1.60
$$
...
```{r}
colors <- c(3, 1, 2, 2, 2, 3, 2, 3, 2, 3, 3, 3, 3, 1, 3, 1, 2, 1, 1, 
            2, 2, 2, 2, 2, 1, 2, 1, 1, 2, 3, 2, 2, 1, 2, 1, 3, 2, 2, 
...
| color | code | $O_i$ | $E_i$ | $\dfrac{(O_i - E_i)^2}{E_i}$ |
|:------:|:----:|:-----:|:-----:|:-----------------------------:|
|   red   |  1    |   15   |  16.67    |   0.1667   |
|   blue   |   2   |   25   |   16.67   |   4.1667   |
|   yellow   |   3   |  10    |   16.67   |  2.6667    |
|      |      |      |  total    |   7.0001   |
...
|:------------:|:---------:|:-------------:|:-----:|
| Carload 1    | 13        | 73            | $n_1 = 86$ |
| Carload 2    | 17        | 57            | $n_2 = 74$ |
| Total        | $c_1 = 30$ | $c_2 = 130$  | $N = 160$ |

The expected table if the carloads and product quality were independent would be:

|              | Defective | Not Defective | Total |
|:------------:|:---------:|:-------------:|:-----:|
| Carload 1 | $\dfrac{86(30)}{160} = 16.125$ | $\dfrac{86(130)}{160} = 69.875$ | $n_1 = 86$ |
| Carload 2 | $\dfrac{74(30)}{160} = 13.875$ | $\dfrac{74(130)}{160} = 60.125$ | $n_2 = 74$ |
| Total | $c_1 = 30$ | $c_2 = 130$ | $N = 160$ |
...
$$
\text{Test statistic} = \frac{(13-16.125)^2}{16.125} + \frac{(73-69.875)^2}{69.875} + \frac{(17-13.875)^2}{13.875} + \frac{(57-60.125)^2}{60.125} = 1.61
$$
...
```{r}
cont.table <- matrix(c(13, 73, 17, 57), ncol=2, byrow=TRUE)
rownames(cont.table) <- c("carload 1", "carload 2")
colnames(cont.table) <- c("Defective", "Not Defective")

cont.table
chisq.test(cont.table, correct = FALSE)
```
...
| Education Level     | Party A | Party B | Party C | Party D | Party E |
|:--------------------|:-------:|:-------:|:-------:|:-------:|:-------:|
| High School         | 50      | 20      | 20      | 30      | 5       |
| Associate's Degree  | 12      | 25      | 15      | 40      | 10      |
| Bachelor's Degree   | 60      | 35      | 25      | 20      | 15      |
| Master's Degree     | 40      | 70      | 30      | 25      | 20      |
...
| Education Level     | Party A | Party B | Party C | Party D | Party E |
|:--------------------|:-------:|:-------:|:-------:|:-------:|:-------:|
| High School         | 35.71   | 33.07   | 19.84   | 25.35   | 11.02   |
| Associate's Degree  | 29.14   | 26.98   | 16.19   | 20.69   | 8.99    |
| Bachelor's Degree   | 44.29   | 41.01   | 24.60   | 31.44   | 13.67   |
| Master's Degree     | 52.86   | 48.94   | 29.37   | 37.52   | 16.31   |

The test statistic is then calculated as the sum of $\dfrac{(O_{ij}-E_{ij})^2}{E_{ij}} = 71.44589$ calculated by:

```{r}
observed <- c(50, 20, 20, 30, 5, 12, 25, 15, 40, 10, 
              60, 35, 25, 20, 15, 40, 70, 30, 25, 20)

expected <- c(35.71, 33.07, 19.84, 25.35, 11.02, 29.14, 26.98, 16.19, 
              20.69, 8.99, 44.29, 41.01, 24.60, 31.44, 13.67, 52.86, 
              48.94, 29.37, 37.52, 16.31)

test_stat <- sum((observed-expected)^2 / expected)
test_stat

1-pchisq(test_stat, df=(5-1)*(4-1))
```
...
```{r}
cont.table <- matrix(c(50,20,20,30,5,
                       12,25,15,40,10,
                       60,35,25,20,15,
                       40,70,30,25,20),
                     nrow=4, byrow=TRUE)

rownames(cont.table) <- c("High School", "Associate's", 
                          "Bachelor's", "Master's")
colnames(cont.table) <- c("Party A", "Party B", "Party C", 
                            "Party D", "Party E")
